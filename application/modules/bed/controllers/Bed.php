...
class Bed extends MX_Controller
{
    ...
    function getBedAllotmentList()
    {
        ...
        foreach ($data['beds'] as $bed) {
            ...
            if ($this->ion_auth->in_group(array('admin', 'Nurse', 'Doctor'))) {
                $option1 = '<a class="btn btn-info btn-xs btn_width editbutton" href="bed/bedAllotmentDetails?id=' . $bed->id . '"><i class="fa fa-edit"> </i> ' . lang('all') . ' ' . lang('details') . '</a>';
            }
            if ($this->ion_auth->in_group(array('admin', 'Nurse', 'Doctor'))) {
            ...
            $option3 = '<a class="btn btn-xs btn-success btn_width invoicebutton" style="margin-right: 10px" target="_blank" href="bed/printLabel?id=' . $bed->id . '"><i class="fa fa-print"> </i> Label Print</a>';
            ...
            $bill_summary_button = '<a class="btn btn-xs btn_width label-warning" style="color: #fff;" target="_blank" href="bed/billDetails?id=' . $bed->id . '"><i class="fa fa-file"> </i> ' . lang('bill') . ' ' . lang('details') . '</a>';
            ...
            if (!empty($servicename)) {
                if ($this->ion_auth->in_group(array('admin'))) {
                    $option .= '<tr id="' . $service->date . '-' . $service_update . '">
                                                        ...
                                                        <td class="no-print" id="delete-service-' . $date_explode[0] . '-' . $servicename->id . '"><button type="button" class="btn btn-danger btn-xs btn_width delete_service" title=' . lang('delete') . ' data-toggle=" "data-id="' . $service->id . "**" . $service_update . '"><i class="fa fa-trash"></i></button></td>
                                                    </tr>';
                } else {
                    if (empty($service->payment_id)) {
                        $option .= '<tr id="' . $service->date . '-' . $service_update . '">
            ...
            if (!empty($servicename)) {
                if ($this->ion_auth->in_group(array('admin'))) {
                    $option .= '<tr id="' . $diagnostic->date . '-' . $diagnostic_update . '">
                                                        ...
                                                        <td class="no-print" id="delete-diagnostic-' . $date_explode[0] . '-' . $servicename->id . '"><button type="button" class="btn btn-danger btn-xs btn_width delete_diagnostic" title=' . lang('delete') . ' data-toggle=" "data-id="' . $diagnostic->id . "**" . $diagnostic_update . '"><i class="fa fa-trash"></i></button></td>
                                                    </tr>';
                } else {
                    if (empty($diagnostic->payment_id)) {
                        $option .= '<tr id="' . $diagnostic->date . '-' . $diagnostic_update . '">
                        ...
                        if (in_array($servicename->id, $pay_diagnostic_new)) {
                            ...
                            $option .= '<tr id="' . $diagnostic->date . '-' . $diagnostic_update . '">
                                                        ...
                                                        <td>' . $diagnostic->quantity . '  </td>
                                                        <td>' . $settings->currency . ' ' . $diagnostic->quantity * $price . '</td>
                                                        <td class="no-print" id="delete-diagnostic-' . $date_explode[0] . '-' . $servicename->id . '"><button type="button" class="btn btn-danger btn-xs btn_width delete_service" title=' . lang('delete') . ' data-toggle=" "data-id="' . $diagnostic->id . "**" . $diagnostic_update . '"><i class="fa fa-trash"></i></button></td>
                                                    </tr>';
                        }
                    }
                }
    ...
    function getServiceByAllotmentId()
    {
        ...
        foreach ($all_services as $service) {
            ...
            $option = '<td class="no-print" id="delete-service-' . date("d") . '-' . $service->id . '"><button type="button" class="btn btn-danger btn-xs btn_width delete_service" title=' . lang("delete") . ' data-toggle="" data-id="' . $service->id . "**" . $service->service . '"><i class="fa fa-trash"></i></button></td>';
            $info[] = array(
                $service_name->name,
    ...
    function getDiagnosticByAllotmentId()
    {
        ...
        foreach ($all_diagnostics as $diagnostic) {
            ...
            $option = '<td class="no-print" id="delete-diagnostic-' . date("d") . '-' . $diagnostic->id . '"><button type="button" class="btn btn-danger btn-xs btn_width delete_diagnostic" title=' . lang("delete") . ' data-toggle="" data-id="' . $diagnostic->id . "**" . $diagnostic->payment_procedure . '"><i class="fa fa-trash"></i></button></td>';
            $info[] = array(
                $payment_procedure,
    ...
    function getMedicineByAllotmentId()
    {
        ...
        foreach ($all_medicines as $medicine) {
            ...
            $option = '<td class="no-print" id="delete-' . $medicine->id . '"><button type="button" class="btn btn-danger btn-xs btn_width delete_medicine" title=' . lang("delete") . ' data-toggle="" data-id="' . $medicine->id . '"><i class="fa fa-trash"></i></button></td>';
            $info[] = array(
